...
In **this** repo, [`.github/workflows/pr-review.yml`](.github/workflows/pr-review.yml) runs the action on pull requests via `uses: ./`.

In a **consuming** repo (after publishing), use:
...
2. Open or update a PR into `main`.
3. Confirm the **PR Review** workflow runs and posts a review on the PR.
## Inputs
