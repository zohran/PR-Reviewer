 ...
 */
function detectLanguage(changedFiles, prDescription) {
 ...
 */
function extractLanguageTag(prDescription) {
    if (!prDescription) {
        return undefined;
    }
...
function voteByExtension(changedFiles) {
 ...
 */
async function fetchPRContext(octokit, context) {
    const { owner, repo } = context.repo;
    const prNumber = context.payload.pull_request?.number;
    if (prNumber === undefined) {
        throw new Error('fetchPRContext requires a pull_request event (payload.pull_request.number is missing)');
    }
...
const rest_1 = __nccwpck_require__(51);
const fetchPR_1 = __nccwpck_require__(241);
const detectLanguage_1 = __nccwpck_require__(458);
const runTests_1 = __nccwpck_require__(229);
const lint_1 = __nccwpck_require__(554);
const llmReview_1 = __nccwpck_require__(744);
const postReview_1 = __nccwpck_require__(585);
...
async function run() {
    try {
        ...
        const failOnLintErrors = parseBoolInput(core.getInput('fail-on-lint-errors'), true);
        const failOnTestFailure = parseBoolInput(core.getInput('fail-on-test-failure'), true);
        ...
        // 3. PR context
        const prContext = await (0, fetchPR_1.fetchPRContext)(octokit, context);
        core.info(`Fetched PR #${prContext.prNumber}: "${prContext.title}" (${prContext.changedFiles.length} files, ${prContext.commitMessages.length} commits)`);
        // 4. Language detection
        const language = (0, detectLanguage_1.detectLanguage)(prContext.changedFiles, prContext.description);
        ...
        const [lintResult, testResult] = await Promise.all([
            (0, lint_1.runLint)(language, workspacePath).catch((error) => {
            ...
            (0, runTests_1.runTests)(language, workspacePath, customTestCommand).catch((error) => {
        ...
        let llmResult;
        try {
            llmResult = await (0, llmReview_1.llmReview)({
            ...
            core.warning(`LLM review failed; continuing with lint/test results only: ${errorMessage(error)}`);
            llmResult = {
                ...EMPTY_LLM_RESULT,
        ...
        const composed = await (0, postReview_1.postReview)(octokit, {
...
function parseBoolInput(raw, defaultValue) {
    if (raw === '') {
        return defaultValue;
    }
...
exports.hasRuffConfig = hasRuffConfig;
exports.hasEslintConfig = hasEslintConfig;
exports.hasGolangciConfig = hasGolangciConfig;
exports.ensureBinary = ensureBinary;
exports.parseRuffJson = parseRuffJson;
exports.parsePylintJson = parsePylintJson;
exports.parseEslintJson = parseEslintJson;
exports.parseGolangciJson = parseGolangciJson;
 ...
 */
async function runLint(language, workingDir) {
...
async function lintPython(workingDir) {
    if (hasRuffConfig(workingDir)) {
        ...
        const output = await runShell('ruff check --output-format=json .', workingDir);
    ...
    const output = await runShell('pylint --output-format=json $(find . -name "*.py" -not -path "./.git/*" -not -path "./.venv/*" -not -path "./venv/*" 2>/dev/null | tr "\\n" " ")', workingDir);
    return { issues: parsePylintJson(output.stdout || output.stderr) };
}
async function lintNode(workingDir) {
...
async function lintGo(workingDir) {
    ...
    const output = await runShell('golangci-lint run --out-format json', workingDir);
...
function hasRuffConfig(workingDir) {
    if (fs.existsSync(path.join(workingDir, 'ruff.toml')) ||
...
function hasEslintConfig(workingDir) {
    return ESLINTRC_PATTERNS.some((name) => fs.existsSync(path.join(workingDir, name)));
}
/** True when a golangci-lint config file exists. */
function hasGolangciConfig(workingDir) {
    return GOLANGCI_CONFIG_FILES.some((name) => fs.existsSync(path.join(workingDir, name)));
}
/**
 ...
 */
async function ensureBinary(binary, workingDir, installArgs) {
    if (await isBinaryAvailable(binary, workingDir)) {
        return true;
    }
    core.warning(`${binary} not found; attempting install: ${installArgs.join(' ')}`);
    ...
    core.warning(`${binary} is still unavailable after install attempt; skipping lint`);
    return false;
}
async function isBinaryAvailable(binary, workingDir) {
    const result = await exec.getExecOutput('bash', ['-lc', `command -v ${shellQuote(binary)}`], {
        cwd: workingDir,
        ignoreReturnCode: true,
        silent: true,
    });
...
async function runShell(command, workingDir) {
...
function parsePylintJson(raw) {
    ...
    return data.map((item) => {
        ...
        const severity = type === 'error' || type === 'fatal' ? 'error' : 'warning';
...
function parseEslintJson(raw) {
    ...
    for (const fileResult of data) {
        ...
        for (const msg of messages) {
            ...
            const severity = severityNum === 2 ? 'error' : 'warning';
            const line = msg.line;
            issues.push({
                file: filePath,
                line: typeof line === 'number' ? line : null,
...
function parseGolangciJson(raw) {
    const data = safeJsonParse(raw);
    ...
    return issuesRaw.map((item) => {
        const pos = (item.Pos ?? item.PosInfo ?? {});
        const line = pos.Line;
        const sev = String(item.Severity ?? 'warning').toLowerCase();
        const severity = sev === 'error' ? 'error' : 'warning';
        return {
...
function formatMessage(message, code) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.MAX_DIFF_CHARS = void 0;
exports.llmReview = llmReview;
exports.buildTruncatedDiff = buildTruncatedDiff;
exports.buildUserMessage = buildUserMessage;
exports.stripMarkdownFences = stripMarkdownFences;
exports.parseAndValidateReview = parseAndValidateReview;
exports.validateReviewResult = validateReviewResult;
/** Soft cap on concatenated diff text sent to the LLM. */
exports.MAX_DIFF_CHARS = 12_000;
const ANTHROPIC_MESSAGES_URL = 'https://api.anthropic.com/v1/messages';
const ANTHROPIC_MODEL = 'claude-sonnet-4-6';
const ANTHROPIC_VERSION = '2023-06-01';
const RETRY_USER_MESSAGE = "Your last response wasn't valid JSON, return only the JSON object.";
 ...
 */
async function llmReview(pr, apiKey) {
    ...
    try {
        ...
        const retryMessages = assistantText
            ? [
                { role: 'user', content: userMessage },
        ...
        try {
            ...
            throw new Error(`LLM review failed after retry. First attempt: ${firstMsg}. Retry: ${secondMsg}`);
        }
    }
}
/**
 ...
 */
function buildTruncatedDiff(changedFiles, maxChars = exports.MAX_DIFF_CHARS) {
    const sections = [];
    let used = 0;
    let included = 0;
    for (const file of changedFiles) {
...
function buildUserMessage(pr) {
    ...
    const commits = pr.commitMessages.length > 0
...
function validateReviewResult(value) {
    ...
    if (typeof obj.matches_intent !== 'boolean') {
        throw new Error('LLM review JSON missing or invalid "matches_intent" (expected boolean)');
    }
    if (typeof obj.summary !== 'string') {
        throw new Error('LLM review JSON missing or invalid "summary" (expected string)');
    }
    if (!Array.isArray(obj.issues)) {
        throw new Error('LLM review JSON missing or invalid "issues" (expected array)');
    }
    const issues = obj.issues.map((item, index) => {
        if (item === null || typeof item !== 'object' || Array.isArray(item)) {
            throw new Error(`LLM review JSON issues[${index}] must be an object`);
        }
        const issue = item;
        if (typeof issue.file !== 'string') {
            throw new Error(`LLM review JSON issues[${index}].file must be a string`);
        }
        if (!(typeof issue.line === 'number' || issue.line === null)) {
            throw new Error(`LLM review JSON issues[${index}].line must be a number or null`);
        }
        if (issue.severity !== 'blocking' && issue.severity !== 'minor') {
            throw new Error(`LLM review JSON issues[${index}].severity must be "blocking" or "minor"`);
        }
        if (typeof issue.comment !== 'string') {
            throw new Error(`LLM review JSON issues[${index}].comment must be a string`);
        }
        return {
            file: issue.file,
            line: issue.line,
...
exports.composeReview = composeReview;
exports.mapLineToDiffPosition = mapLineToDiffPosition;
 ...
 */
async function postReview(octokit, pr, payload) {
 ...
 */
function composeReview(payload) {
    const { lintIssues, testResult, llmResult, failOnLintErrors, failOnTestFailure, changedFiles, } = payload;
    const blockingLintIssues = failOnLintErrors
        ? lintIssues.filter((issue) => issue.severity === 'error')
        : [];
    const testsFailed = failOnTestFailure && testResult.ran && testResult.passed === false;
    const blockingLlmIssues = llmResult.issues.filter((issue) => issue.severity === 'blocking');
    const event = blockingLintIssues.length > 0 ||
        testsFailed ||
 ...
 */
function mapLineToDiffPosition(patch, targetLine) {
    ...
    for (const line of lines) {
        ...
        if (hunk) {
            ...
            const newLen = hunk[4] !== undefined ? Number.parseInt(hunk[4], 10) : 1;
...
function tryAnchorOrFold(args) {
    const { file, line, body, patchByFile, comments, unanchoredIssues, label } = args;
    if (line == null) {
    ...
    if (!patch) {
        unanchoredIssues.push(`**${label}** \`${file}:${line}\`: ${body} _(no diff patch)_`);
        return;
    }
    const position = mapLineToDiffPosition(patch, line);
    if (position == null) {
        unanchoredIssues.push(`**${label}** \`${file}:${line}\`: ${body} _(line not in diff)_`);
        return;
    }
    comments.push({
...
function buildReviewBody(args) {
    ...
    sections.push(args.matchesIntent
...
function formatLintComment(issue) {
    return `**Lint ${issue.severity}:** ${issue.message}`;
}
function formatLlmComment(issue) {
    return `**${issue.severity === 'blocking' ? 'Blocking' : 'Minor'}:** ${issue.comment}`;
}
function buildPatchLookup(changedFiles) {
    const map = new Map();
    for (const file of changedFiles) {
        if (typeof file.patch === 'string') {
...
function findPatchForFile(patchByFile, file) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.MAX_TEST_OUTPUT_CHARS = void 0;
exports.runTests = runTests;
exports.hasAnyConfigFile = hasAnyConfigFile;
exports.capOutput = capOutput;
exports.getLanguageTestPlan = getLanguageTestPlan;
const fs = __importStar(__nccwpck_require__(24));
const path = __importStar(__nccwpck_require__(760));
const exec = __importStar(__nccwpck_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module '@actions/exec'"); e.code = 'MODULE_NOT_FOUND'; throw e; }())));
/** Soft cap on captured test output kept for PR comments / LLM context. */
exports.MAX_TEST_OUTPUT_CHARS = 5000;
 ...
 */
async function runTests(language, workingDir, customTestCommand) {
 ...
 */
function hasAnyConfigFile(workingDir, configFiles) {
    return configFiles.some((file) => fs.existsSync(path.join(workingDir, file)));
}
/**
 * Keeps the last `maxChars` of output so failure details (usually at the end)
 * are preserved for PR comments.
 */
function capOutput(output, maxChars = exports.MAX_TEST_OUTPUT_CHARS) {
 ...
 */
function getLanguageTestPlan(language) {
    return LANGUAGE_PLANS[language.toLowerCase()];
}
async function runCommandSequence(commands, workingDir) {
    let combined = '';
    let lastExitCode = 0;
